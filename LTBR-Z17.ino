...
// Setup Flags
bool clrEEPROM;                 // HIGH to clear EEPROM memory, Readed from EEPROM in setup()
bool fillEEPROM;                // HIGH to fill the EEPROM with default values, Readed from EEPROM in setup()
bool bootMode = LOW;            // flag to trigger qtr calibration or use EEPROM values.
...
void setup()
{
  ...
  ReadStartUpFlagsFromEEPROM ();
  EraseEEPROM ();                     // Checks clrEEPROM flag & clears EEPROM if needed.
  ...
  ToggleLED (HIGH, HIGH);             // Both LEDs ON: calibration can be started pressing the buton.
